...
_this_script=/opt/brandt/init.d/syslog
...
function syslog_service() {
	...
	then
		...
		( echo -e "# send everything to syslog host (Bob Brandt)"
		  ...
		  echo -e " udp(\"syslog.opw.ie\" port(514));"
		  echo -e "# tcp(\"syslog.opw.ie\" port(514));"
		  echo -e "};\nlog {\n source(src);"
		  echo -e " destination(loghost);\n};" ) >> "$_conf_syslog"
		brandt_status setup
	fi
...
function eDirectory_logs() {
	folders=""
	test -d "/var/opt/novell/log" && folders="/var/opt/novell/log "
	test -d "/var/opt/novell/tomcat5/logs" && folders="$folders/var/opt/novell/tomcat5/logs "
	test -d "/var/opt/novell/eDirectory/log" && folders="$folders/var/opt/novell/eDirectory/log "
	test -d "/var/opt/novell/eDirectory/data/embox/log" && folders="$folders/var/opt/novell/eDirectory/data/embox/log "
	test -d "/var/opt/novell/iManager/nps/WEB-INF/logs" && folders="$folders/var/opt/novell/iManager/nps/WEB-INF/logs "
	test -d "/var/opt/novell" && folders="$folders/var/opt/novell/*/*.log /var/opt/novell/*/*/*.log /var/opt/novell/*/*/*/*.log "
	if [ ! "$folders" == "" ]; then
		echo -n "Setting up Logrotate services "
		echo -e "$folders{\n    maxage 90\n    rotate 1\n    missingok\n    notifempty\n    size +1024k\n}" > /etc/logrotate.d/brandt-oes2
		brandt_status setup
	fi
}
